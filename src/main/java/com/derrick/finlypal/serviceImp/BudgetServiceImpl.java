...
public class BudgetServiceImpl implements BudgetService {
  ...
  public GeneralResponseDTO markBudgetAsCompleted(Long budgetId)
    ...
    try {
      ...
      if (budget.getStatus().equals(BudgetStatus.COMPLETED)) {
  ...
  private String getStatusTooltip(BudgetStatus status) {
    return switch (status) {
      ...
      case EXCEEDED ->
          "🚨 This budget is still active, but the allocated amount has already been used up!";
      case EXPIRED ->
      ...
      case UNDERUTILIZED ->
          "📉 Less than 50% of the budget was used. Consider adjusting future allocations.";
    };
  }
}
