...
public class BudgetServiceImpl implements BudgetService {
    ...
    /**
     * Creates a new budget for the logged-in user. It validates the provided budget details,
     * such as ensuring the start date is before the end date and that both dates are not in the past.
     * If the budget details are valid, it calculates the budget status and saves the budget to the
    ...
    /**
     * Updates a budget with the given {@link BudgetRequestDTO}. It takes in the id of the budget to be
     * updated and the {@link BudgetRequestDTO} containing the new values. It returns a {@link
     * BudgetResponseDTO} indicating the status of the request. If the request is successful, it returns a
     * status of {@link HttpStatus#OK} and a message indicating that the budget was successfully updated.
     * If the budget with the given id is not found, it throws a {@link NotFoundException}. If the user is
     * not authorized to update the budget, it throws a {@link NotAuthorizedException}. If any unexpected
     * error occurs while trying to update the budget, it throws an {@link InternalServerErrorException}.
     * <p>
     * It also validates that the start date is before the end date, and that the budget is not in the
     * past. If the budget is completed, it throws a {@link BadRequestException}.
     *
    ...
    /**
     * Retrieves a budget by its ID for the logged-in user. This method ensures that the budget
     * exists and belongs to the logged-in user, throwing exceptions if these conditions are not met.
     * It also updates the budget's status based on its start and end dates and total spent amount.
     * The response includes the budget's details along with its associated budget items.
     *
    ...
    /**
     * Deletes a budget by its ID for the logged-in user. This method ensures that the budget
     * exists and belongs to the logged-in user, throwing exceptions if these conditions are not met.
     * Upon successful deletion, it returns a {@link GeneralResponseDTO} with a status of {@link HttpStatus#OK}
     * and a message indicating the successful deletion of the budget. If the budget with the given ID
     * is not found, it throws a {@link NotFoundException}. If the user is not authorized to delete
     * the budget, it throws a {@link NotAuthorizedException}. If any unexpected error occurs while
     * trying to delete the budget, it throws an {@link InternalServerErrorException}.
     *
     ...
     * @throws InternalServerErrorException if any unexpected error occurs while trying to delete the budget
     */
    @Override
     ...
     * {@link BudgetItem}s associated with the budget. This method iterates over the budget items,
     * and for each item, it fetches the total amount of money spent using the
     * {@link ExpenseRepository#getTotalExpenseByBudgetItem(Long)} method. The total amount spent is
     * then returned as a {@link BigDecimal}.
     *
     ...
     *
     * <ul>
     *   <li>PLANNED: The budget is scheduled to start in the future.</li>
     *   <li>EXCEEDED: The budget period has ended, and the total spent exceeds the allocated budget.</li>
     *   <li>EXPIRED: The budget period has ended, but the total spent is within the allocated budget.</li>
     *   <li>IN_PROGRESS: The budget is currently active.</li>
     * </ul>
     *
