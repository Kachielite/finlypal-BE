...
public class SavingsServiceImpl implements SavingsService {
   ...
   * successful, it returns a {@link SavingsResponseDTO} with a status of {@link HttpStatus#OK} and
   * a message indicating the successful update of the savings goal. If the savings goal with the
   * provided id is not found, it throws a {@link NotFoundException}. If the user is not throws a
   * {@link NotFoundException}. If the user is not authorized to update the savings goal, it throws
   * a {@link NotAuthorizedException}. If any unexpected error occurs while trying to update the
  ...
  public SavingsResponseDTO updateSavings(Long savingsId, SavingsRequestDTO savingsRequestDTO)
    ...
    try {
      ...
      savings.setIcon(savingsRequestDTO.icon());
  ...
  public SavingsResponseDTO getSavingsById(Long savingsId)
    ...
    try {
      ...
      SavingsStatus status =
          getSavingsStatus(
              savings.getStartDate(), savings.getEndDate(), savings.getTargetAmount(), savedAmount);
  ...
  private SavingsStatus getSavingsStatus(
    ...
    BigDecimal progress =
        savedAmount.divide(targetAmount, 2, RoundingMode.HALF_UP); // Normalized progress
